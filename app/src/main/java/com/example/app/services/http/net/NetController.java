...
//            byte[]requestBodyBytes=request.getBody();
//            if(requestBodyBytes==null||requestBodyBytes.length==0){
//                return ResponseContext.status(400).contentType("application/json").body("{\"status\":\"error\",\"message\":\"Empty request body received by proxy broker.\"}").build();
//            }
//            String jsonConfig=new String(requestBodyBytes,StandardCharsets.UTF_8);
//            JSONObject rootEnvelope=new JSONObject(jsonConfig);
//            int customTimeoutMs=15000;
...
//            }
//            if(!rootEnvelope.has("request")){
//                return ResponseContext.status(400).contentType("application/json").body("{\"status\":\"error\",\"message\":\"Missing 'request' object containing delivery metadata.\"}").build();
//            }
//            JSONObject innerRequest=rootEnvelope.getJSONObject("request");
//            String targetUrl=innerRequest.optString("url","");
//            String method=innerRequest.optString("method","POST").toUpperCase();
//            if(targetUrl.isEmpty()){
//                return ResponseContext.status(400).contentType("application/json").body("{\"status\":\"error\",\"message\":\"Missing target destination 'url' parameter inside request metadata block.\"}").build();
//            }
//            Log.i(TAG," -> Unwrapping request envelope and executing curl-like dispatch to: "+targetUrl);
//            java.net.URL url=new java.net.URL(targetUrl);
//            java.net.HttpURLConnection conn=(java.net.HttpURLConnection)url.openConnection();
//
//            // BYPASS FOR ALL API LEVELS: Overriding certificate chain validation unconditionally
//            if(conn instanceof javax.net.ssl.HttpsURLConnection){
//                Log.w(TAG,"[ZEBRA BROKER SSL RECOVERY] Overriding certificate chain validation for connection.");
//                javax.net.ssl.HttpsURLConnection sslConn=(javax.net.ssl.HttpsURLConnection)conn;
//                javax.net.ssl.TrustManager[]trustAllCerts=new javax.net.ssl.TrustManager[]{
//                    new javax.net.ssl.X509TrustManager(){
//                        public java.security.cert.X509Certificate[]getAcceptedIssuers(){return null;}
...
//                    if(key==null)continue;
//                    String lowerKey=key.toLowerCase();
//                    if(lowerKey.equals("host")||lowerKey.equals("content-length")||lowerKey.equals("connection")||lowerKey.equals("accept-encoding")){
//                        continue;
//                    }
//                    conn.setRequestProperty(key,customHeaders.getString(key));
//                }
...
//        }catch(Exception e){
//            Log.e(TAG,e.toString());
//            return ResponseContext.status(500).contentType("application/json").body("{\"status\":\"error\",\"message\":\"Native broker envelope routing failed: "+e.getMessage()+"\"}").build();
//        }
//    }
//
//    // Note: The rest of downloadFileRemote is omitted here as it was cut off in your snippet, 
//    // but you should implement the same conditional block removal inside its redirection loop if it makes HttpsUrlConnections.
//}
//--------------------------------------------------------------------------------
package com.example.app.services.maintenance;
