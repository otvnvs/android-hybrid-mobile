#!/usr/bin/env python3
"""
Host-side test harness for the AHM background service optimized for WSL.

The Android app connects to --android-port (default 9999) over the Windows adb reverse
...
# Point to your custom Windows ADB path inside WSL
WSL_ADB_PATH = "/mnt/c/usr/bin/adb.exe"
...
class TestServer:
    ...
    async def run(self):
        ...
        try:
            ...
            print("\n[server] Shutting down sockets and clean dependencies...")
    ...
    async def _adb_reverse(self, action: str):
        ...
        # Direct the subprocess execution to use the Windows binary
        cmd = [WSL_ADB_PATH, "reverse"]
        if action == "remove":
            cmd += ["--remove", spec]
        else:
            cmd += [spec, spec]
        try:
            ...
            if proc.returncode == 0:
                ...
                err_msg = stderr.decode("utf-8", errors="ignore").replace("\r\n", "\n").strip()
                print(f"[adb] reverse {action} failed: {err_msg}")
        except FileNotFoundError:
            print(f"[adb] Windows ADB executable not found at {WSL_ADB_PATH}; reverse tunnel must be set up manually.")
...
async def async_main():
    ...
    parser.add_argument(
        "--control-socket",
        type=Path,
        default=Path(os.environ.get("AHM_TEST_CONTROL_SOCKET", f"/tmp/{CONTROL_SOCKET_NAME}")),
        help="Unix socket path for local control clients (Defaults to /tmp/ for WSL compatibility).",
    )
    parser.add_argument(
        "--no-reverse",
        action="store_true",
    ...
    if not args.control_socket.is_absolute():
        if "/mnt/" in str(Path(__file__).resolve()):
            args.control_socket = Path(f"/tmp/{CONTROL_SOCKET_NAME}")
        else:
    ...
    # Register standard background signal tracking
    ...
    # Wrap the running engine inside a try/except specifically watching for KeyboardInterrupt
    try:
        await server.run()
    except KeyboardInterrupt:
        print("\n[server] Interrupted via keyboard shortcut.")
        server.request_shutdown()
        await server.run() # Triggers the structural cleanup sequence inside server.run()
